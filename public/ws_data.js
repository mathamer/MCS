...
function GenerateUpdateDataCallback(chart){
    return (data, label)=>{
        ...
        if(chart.data.datasets[0].data.length > MAX_CHART_LABELS_LEN) chart.data.datasets[0].data.shift();
...
function UpdateGraphsFromData(dataArray, label){
    ...
    for(let c = 0; c<UpdateDataCallback.length; c++){
        ...
        if(fullChart.data.datasets[c].data.length > MAX_CHART_LABELS_LEN) fullChart.data.datasets[c].data.shift();
    }
